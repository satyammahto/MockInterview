...
async def start_session(
    ...
    try:
        ...
        print("Generating questions for skills:", skills) # Keep existing print for debugging
        ...
        if not isinstance(questions_data, list): # Keep existing validation
            raise HTTPException(status_code=500, detail="Invalid response from question generator.")
        ...
        db_session = InterviewSession(
            ...
            num_questions=num_questions, # Changed from str(num_questions) to num_questions as per original
            status="in_progress",
        )
        db.add(db_session)
        db.flush()
        ...
        for i, q in enumerate(questions_data):
            db_q = Question(
                ...
                text=q.get("text", ""), # Changed from q["text"] to q.get("text", "") as per original
                type=q.get("type", "technical"), # Changed from q["type"] to q.get("type", "technical") as per original
                order_index=i,
            )
            db.add(db_q)
        ...
        print("ERROR in /sessions/start:", str(e))
